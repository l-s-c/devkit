// sidebar.js — 方案B: 200px侧边栏，内容区完全不改
// 断点≥1400px，浮在右侧空白区域，零侵入
(function(){
  ...
  style.textContent = `
.dk-sidebar{width:200px;display:none;position:fixed;right:24px;top:80px;max-height:calc(100vh - 96px);overflow-y:auto;scrollbar-width:none;z-index:40}
.dk-sidebar::-webkit-scrollbar{display:none}
@media(min-width:1400px){.dk-sidebar{display:block}}
...
.dk-ad-slot{border-radius:8px;overflow:hidden;width:160px;height:600px;margin:0 auto;display:flex;align-items:center;justify-content:center;border:1px dashed var(--border-glass,rgba(0,0,0,0.08));background:linear-gradient(135deg,rgba(99,102,241,0.06),rgba(168,85,247,0.06))}
.dk-ad-label{font-size:9px;color:var(--text-caption,#9CA3AF);font-weight:600;letter-spacing:1px;writing-mode:vertical-lr}
...
`;
  ...
  aside.innerHTML = `
    ...
    <div class="dk-sb-section" style="padding:8px;overflow:hidden">
      <div class="dk-ad-slot"><span class="dk-ad-label">📢 广告 160×600</span></div>
    </div>
    <div class="dk-sb-section">
  ...
  if (mainEl) {
    aside.style.top = mainEl.offsetTop + 'px';
  }
